...
```
```{r}
focotreesraw <- read.csv("/Users/freddiehaberecht/Desktop/R_Class/ESS523_2024/Final Project/focoall.csv")
```
```{r}
focotrees <- focotreesraw %>%
  rename(CommonName = Species) %>%
  mutate(ScientificName = gsub(".*\\((.*)\\).*", "\\1", CommonName)) %>%
  separate(ScientificName, into = c("Genus", "Species"), sep = " ", extra = "merge") %>%
  mutate(Species = word(Species, 1)) %>%
  filter(!Genus %in% c("Stump", "Vacant", "Malus'")) %>%
  select(Latitude, Longitude, Genus, Species)
```
```{r}
save(focotrees, file = "focotrees.RData")
```

```{r}
focopoints <- focotrees %>%
  st_as_sf(coords = c("Longitude", "Latitude"), crs = 4326)
```
```{r}
save(focopoints, file = "focopoints.RData")
```
